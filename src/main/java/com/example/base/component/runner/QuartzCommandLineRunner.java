...
//        CronScheduleBuilder.cronSchedule()
        ...
        JobDetail jobDetail = JobBuilder.newJob((Class<QuartzJobBean>) Class.forName("com.example.base.running.job.DemoQuartzJob"))
                .withIdentity("demoQuartzJob").storeDurably().build();
        scheduler.addJob(jobDetail, true);


//        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule("0/10 * * * * ? *");
//        Trigger trigger = TriggerBuilder.newTrigger().forJob(jobDetail).withIdentity("demoTriggerCC").withSchedule(cronScheduleBuilder).build();
//        scheduler.scheduleJob(trigger);

        TriggerKey triggerKey = new TriggerKey("demoTriggerCC");
